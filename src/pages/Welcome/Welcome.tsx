...
function WelcomeMobile({ form, onSubmit, loading, error }: WelcomeMobileProps) {
  return (
    <div className="m-stack" id="contenido">
      ...
      <p className="m-sub">¡Bienvenidos a bordo</p>
      <p className="m-title">Pequeños piratas!</p>

      <form className="welcome__form welcome__form--mobile" onSubmit={onSubmit} noValidate>
        <h2 className="welcome__title">Ingresá tus datos para participar</h2>
        <div className="welcome__fields">{form}</div>
        {error && (
          <p className="welcome__error" role="alert">
            {error}
        ...
        <PromoButton type="submit" fontSize={60} loading={loading}>
          Participar
        </PromoButton>
        <Link className="welcome__help" to="/donde-esta-el-codigo">
          ¿Dónde encuentro el código secreto?
        </Link>
      </form>

      <PurosolShip className="m-art" style={{ position: 'relative', height: 'auto', aspectRatio: '1266 / 867' }} />
    </div>
  );
}
